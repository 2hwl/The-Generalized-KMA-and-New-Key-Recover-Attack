...
int oracle(const uint8_t ciphertext[SABER_BYTES_CCA_DEC], const uint8_t sk[SABER_INDCPA_SECRETKEYBYTES], int * msg)
{
	uint16_t s[SABER_L][SABER_N];
	uint16_t b[SABER_L][SABER_N];
	uint16_t v[SABER_N] = { 0 };
	...
	for (i = 0; i < SABER_N; i++)
	{
		v[i] = ((v[i] + h2 + 1024000 - (cm[i] << (SABER_EP - SABER_ET))) % 1024) >> (SABER_EP - 1);
		msg[i] = v[i];
	}
