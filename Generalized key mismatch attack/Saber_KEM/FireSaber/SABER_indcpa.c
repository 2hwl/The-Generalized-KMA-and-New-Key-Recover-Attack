...
int oracle(const uint8_t ciphertext[SABER_BYTES_CCA_DEC], const uint8_t sk[SABER_INDCPA_SECRETKEYBYTES], uint8_t * msg)
{
	uint16_t s[SABER_L][SABER_N];
	uint16_t b[SABER_L][SABER_N];
	uint16_t cm[SABER_N];
	...
	//printf("\nv[0] = %d\n",  ((v[0] + h2 - (cm[0] << (SABER_EP - SABER_ET))) % 1024) >> (SABER_EP - 1));
	//printf("v[0] = %d, h2= %d, cm = %d, ep = %d \n\n",  v[0] , h2,  ((- (cm[0] << (SABER_EP - SABER_ET))) % 1024) , (SABER_EP - 1));
	
	for (t = 0; t < SABER_N; t++)
	{
		v[t] = ((v[t] + h2 + 1024000 - (cm[t] << (SABER_EP - SABER_ET))) % 1024) >> (SABER_EP - 1);
		msg[t] = v[t];
	}
	POLmsg2BS(m1, v);
	return (m1[0] & 1);
