...
static int kyber_Attack(int P) {
    ...
    for(int i = 0; i < KYBER_K; i++) {
        for(int k = 0; k < KYBER_N; k+=P) {
            ...
            kemenc_Attack(ct, m, 228, C2, k, i, P); 
            query += 1;
    ...
    for(int i=0;i<KYBER_K;i++){
    	for(int j=0; j<KYBER_N; j++){
    		...
    		memset(C2, 0, sizeof(C2));
    	    	...
    	    	kemenc_Attack(ct, m, 228, C2, j, i, -1); 
		query += 1;
    ...
    for(int i=0;i<KYBER_K;i++){
		...
		oracle(ct, sk, m, j, -1, C3);
    	    	...
    	    	if(C3[j] == 0) recs[i][j] = 3;
    	    	else recs[i][j] = 2;
    	    	
    	    	if(cnt != 0){
    	    	    for(int k=0; k<cnt; k++){
    	    	      if(C3[index[k]] == 0) recs[i][index[k]] = 3;
    ...
    for(int i=0;i<KYBER_K;i++){
		...
		oracle(ct, sk, m, j, -1, C3);
    	    	...
    	    	if(C3[j] == 0) recs[i][j] = 0;
    	    	else recs[i][j] = 10;
    	    	
    	    	if(cnt != 0){
    	    	    for(int k=0; k<cnt; k++){
    	    	      if(C3[index[k]] == 0) recs[i][index[k]] = 0;
    	    	      else recs[i][index[k]] = 10;
    	    	    }
    	    	}
    	    }
    	}
