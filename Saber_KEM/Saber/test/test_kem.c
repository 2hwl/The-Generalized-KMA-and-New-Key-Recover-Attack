...
static int test_pke_cpa(int P)
{
    ...
    for(int i=0; i<SABER_L; i++){
    	...
    	for(int k=0; k<SABER_N; k+=P){
    		...
    		if(msg[t] == 0) vprime[t] = 5, che[i][t][0] = 0;
    		else vprime[t] = 3, che[i][t][0] = 1;
    	    }
    ...
    for(int i=0; i<SABER_L; i++){
		...
		query += 1;
    	    	...
    	    	if(msg[j] == 1) sk_hat[i][j] = 2;
    	    	else sk_hat[i][j] = 10;
    	    	
    	    	if(cnt != 0){
    	    	    for(int k=0; k<cnt; k++){
    	    	      if(msg[index[k]] == 1) sk_hat[i][index[k]] = 2;
    ...
    for(int i=0;i<SABER_L;i++){
		...
		query += 1;
    	    	...
    	    	if(msg[j] == 0) sk_hat[i][j] = 4;
    	    	else sk_hat[i][j] = 3;
    	    	
    	    	if(cnt != 0){
    	    	    for(int k = 0; k < cnt; k ++){
    	    	      if(msg[index[k]] == 0) sk_hat[i][index[k]] = 4;
